...
function GetSummonerInformation($conn, $summonerName, $region) {
	...
	$summoner = new Summoner ( $result ["Id"], $result ["Name"], $result ["Region"], $result ["ProfileIconId"], $result ["RevisionDate"], $result ["SummonerLevel"], $result ["MasteriesExist"], $result ["LastSearched"] );
...
function GetChampionMastery($conn, $summonerId) {
	...
	foreach ( $masteryInformation->fetchAll () as $row ) {
		array_push ( $championMastery, new ChampionMastery ( $row ["ChampionId"], $row ["ChampionLevel"], $row ["ChampionPoints"], $row ["ChampionPointsSinceLastLevel"], $row ["ChampionPointsUntilNextLevel"], $row ["ChestGranted"], $row ["HighestGrade"], $row ["LastPlayTime"], $row ["PlayerId"] ) );
	}
